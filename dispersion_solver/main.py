...
from util.iaw import eps_IAW, analytic_IAW, analytic_IAW_simple,first_guess,first_guess_1,precedent_guess,first_guess_mod
...
primo = True
kzetas = np.arange(0.001,0.006,0.001)
...
for i,kz in enumerate(kzetas):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(path + current + "/images_dispersion/dispersion_kz={:4.3f}_gamma.png".format(kz))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(path + current + "/images_dispersion/dispersion_kz={:4.3f}_omega.png".format(kz))
    plt.close()
    ...
    plt.savefig(path + current + "/images_dispersion/error_kz={:4.3f}".format(kz) + ".png")
    plt.close()
...
for i,kz in enumerate(kzetas):
    ...
    # plt.plot(kysref1, dispersion[i,1,:], "green", label="$\omega_r$ solver")
    plt.plot(ky_1, gamma_1, label="$\gamma$ solver, kz = %4.3f"%kz)
    plt.xlabel("Azimuthal wave number $k_{\\theta} \\lambda_{De}$")
    plt.ylabel("Pulsations  $\\gamma/\\omega_{pi}$ ")
